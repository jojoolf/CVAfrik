  ...
  return (
    <div className="min-h-full bg-slate-950 px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900 to-primary/20 p-6 shadow-2xl sm:p-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Diffuse une annonce ou une opportunité depuis ton espace administrateur, sans accéder aux outils techniques.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] px-5 py-4">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Audience disponible</p>
              ...
              <p className="mt-1 text-xs text-slate-500">comptes CVAfrik</p>
            </div>
          </div>
        </header>

        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
          <Card className="border-white/10 bg-slate-900/80 shadow-2xl shadow-black/15">
            <CardHeader><CardTitle className="flex items-center gap-2 text-white"><BellRing className="h-5 w-5 text-primary" /> Nouvelle diffusion</CardTitle><CardDescription>Le message est ajouté à la cloche de chaque compte autorisé avant la remise push ou e-mail.</CardDescription></CardHeader>
            <CardContent>
              <form className="space-y-5" onSubmit={publish}>
                <div className="space-y-2"><Label htmlFor="notification-category">Catégorie</Label><select id="notification-category" value={category} onChange={(event) => changeCategory(event.target.value as CategoryId)} className="h-11 w-full rounded-md border border-white/10 bg-slate-950 px-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary"><option value="announcement">Annonce importante</option><option value="opportunity">Opportunité</option></select><p className="text-xs text-slate-500">{selectedCategory.description}</p></div>
                <div className="space-y-2"><div className="flex items-center justify-between"><Label htmlFor="notification-title">Titre</Label><span className="text-xs text-slate-500">{title.length}/180</span></div><Input id="notification-title" value={title} maxLength={180} onChange={(event) => setTitle(event.target.value)} placeholder="Ex. Une nouvelle opportunité est disponible" className="h-11 border-white/10 bg-slate-950 text-white placeholder:text-slate-600" required /></div>
                <div className="space-y-2"><div className="flex items-center justify-between"><Label htmlFor="notification-body">Message</Label><span className="text-xs text-slate-500">{body.length}/600</span></div><Textarea id="notification-body" value={body} maxLength={600} onChange={(event) => setBody(event.target.value)} placeholder="Écris un message court, clair et utile pour les utilisateurs." className="min-h-36 border-white/10 bg-slate-950 text-white placeholder:text-slate-600" required /></div>
                <div className="space-y-2"><Label htmlFor="notification-link">Page ouverte au clic</Label><Input id="notification-link" value={href} onChange={(event) => setHref(event.target.value)} placeholder="/dashboard ou /opportunites" className="h-11 border-white/10 bg-slate-950 text-white placeholder:text-slate-600" /><p className="text-xs text-slate-500">Utilise uniquement une page interne commençant par « / ».</p></div>
          ...
          <aside className="space-y-6">
            <Card className="border-primary/25 bg-primary/10"><CardHeader><CardTitle className="text-lg text-white">Ce que reçoit chaque compte</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-300"><p className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />Une entrée dans la cloche et l’historique CVAfrik.</p><p className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />Une alerte Android, si la personne l’a activée.</p><p className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />Un e-mail de secours, si ce choix est activé.</p></CardContent></Card>
            <Card className="border-white/10 bg-slate-900/80"><CardHeader><CardTitle className="flex items-center gap-2 text-lg text-white"><Info className="h-4 w-4 text-primary" /> Respect des préférences</CardTitle></CardHeader><CardContent><p className="text-sm leading-6 text-slate-400">Les utilisateurs qui ont désactivé les annonces ou les opportunités ne reçoivent pas cette catégorie. La diffusion est enregistrée dans le journal administrateur.</p></CardContent></Card>
            {summary && <Card className="border-emerald-400/30 bg-emerald-400/10"><CardHeader><CardTitle className="text-lg text-white">Dernière diffusion</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-slate-200"><p><Badge className="mr-2 bg-emerald-500/20 text-emerald-200 hover:bg-emerald-500/20">{summary.inbox}</Badge>historiques créés</p><p><Badge className="mr-2 bg-sky-500/20 text-sky-200 hover:bg-sky-500/20">{summary.push}</Badge>alertes Android remises</p><p><Badge className="mr-2 bg-violet-500/20 text-violet-200 hover:bg-violet-500/20">{summary.email}</Badge>e-mails envoyés</p>{summary.skipped > 0 && <p className="text-xs text-slate-400">{summary.skipped} compte(s) ont choisi de ne pas recevoir cette catégorie.</p>}</CardContent></Card>}
          </aside>
        </div>
      </div>
    </div>
