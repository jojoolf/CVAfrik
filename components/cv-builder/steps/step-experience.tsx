...
export function StepExperience({ data, onUpdate }: StepExperienceProps) {
  ...
  const addRealisation = (expId: string) => {
    ...
    if (exp) {
      updateExperience(expId, {
        realisations: [...exp.realisations, realisation],
      })
  ...
  const removeRealisation = (expId: string, index: number) => {
    ...
    if (exp) {
      updateExperience(expId, {
        realisations: exp.realisations.filter((_, i) => i !== index),
      })
    }
  }
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="Ex: Orange Cote d'Ivoire"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => 
                      updateExperience(experience.id, { en_cours: checked as boolean })
                    }
                  />
                  <Label htmlFor={`en_cours_${experience.id}`} className="text-sm">
                    Poste actuel
                  </Label>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Decrivez vos responsabilites principales..."
                    rows={3}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => 
                        setNewRealisation(prev => ({ ...prev, [experience.id]: e.target.value }))
                      }
                      placeholder="Ex: Augmentation des ventes de 30%"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault()
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {experience.realisations.map((realisation, index) => (
                        <Badge key={index} variant="secondary" className="gap-1">
                          ...
                          <button
                            type="button"
                            onClick={() => removeRealisation(experience.id, index)}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Ajoutez des resultats concrets et mesurables
                  </p>
                </div>
              </CardContent>
            </Card>
