...
function safeAspectRatio(value: number) {
  return Math.min(Math.max(value, 0.45), 2.4)
}
...
export function InAppCampaignModal() {
  ...
  const displayAspectRatio = imageAspectRatio || 3 / 4

  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/65 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="campaign-title">
    <div className="max-h-[92dvh] w-full max-w-[390px] overflow-y-auto rounded-[2rem] border border-white/25 bg-card shadow-2xl">
      <div className="relative max-h-[60dvh] w-full overflow-hidden bg-muted" style={{ aspectRatio: displayAspectRatio }}>
        <img src={campaign.image_url} alt={campaign.title} onLoad={(event) => setImageAspectRatio(safeAspectRatio(event.currentTarget.naturalWidth / event.currentTarget.naturalHeight))} className="h-full w-full object-contain" />
        <Button type="button" variant="secondary" size="icon" onClick={close} className="absolute right-3 top-3 h-10 w-10 rounded-full bg-background/90 shadow-lg" aria-label="Fermer la campagne"><X className="h-5 w-5" /></Button>
      </div>
