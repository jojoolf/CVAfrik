            ...
            <SheetContent side="bottom" showClose={false} className="max-h-[78dvh] overflow-hidden rounded-t-[1.75rem] px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3">
              ...
              <div className="min-h-0 overflow-y-auto overscroll-contain pb-2">
                {moreView === 'menu' && <>
                  ...
                  {isAdmin && <Link href="/admin" onClick={closeMoreMenu} className="mt-4 flex h-auto w-full items-center rounded-2xl border border-primary/35 bg-primary px-4 py-3 text-left text-primary-foreground shadow-lg shadow-primary/20 transition active:scale-[0.99]"><ShieldCheck className="mr-3 h-5 w-5" /><span><span className="block text-sm font-black">Ouvrir mon Administration</span><span className="mt-0.5 block text-xs font-normal text-primary-foreground/80">Gérer CVAfrik directement depuis ce téléphone</span></span></Link>}
